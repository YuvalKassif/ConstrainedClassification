...
from losses import SpoPlusLoss
...
def main():
    ...
    # Device selection (simple, consistent with main.py defaults)
    ...
    true_test_counts = _compute_counts(test_loader, num_classes)
    total_test = sum(true_test_counts) if true_test_counts else len(test_loader.dataset)
    ...
    for percent in constraints_percentage_list:
        N_K_val = true_test_counts[constrained_class_index] * percent / 100.0
        constraint_ratio = float(N_K_val) / float(total_test) if total_test else 0.0
        ...
        spo_results = constrained_classification(
            current_model,
            None,
            test_loader,
            ...
            N_K_val,
            device,
            save_path="raw_results_SPOplus.csv",
        )

        results = {
            "SPOplus_results": spo_results,
            "counts": class_count,
        ...
        save_parameters(
            ...
            base_timestamp,
            iteration_timestamp,
            filename="results_spo_plus.json",
            dataset=params.get("dataset", "unknown_dataset"),
            constrained_class_index=constrained_class_index,
        )
